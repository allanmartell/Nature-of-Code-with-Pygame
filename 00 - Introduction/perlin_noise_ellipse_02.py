...
# REQUIRED
scalex = random.random() # determines at what distance to view the noisemap.
...
def draw():
	...
	nx = pnoise1(scalex, octaves=octaves, lacunarity=lacunarity)
	ny = pnoise1(scaley, octaves=octaves, lacunarity=lacunarity)
	print('nx: ', nx, 'ny: ', ny) 
	x = linearConversion(nx, (-1, 1), (0, WIDTH))
	y = linearConversion(ny, (-1, 1), (0, HEIGHT))
	print('x: ', x, 'y: ', y)
	
	# stroke
	ellipse_stroke = pygame.Rect(x-stroke//2, y-stroke//2, ellipse_size+stroke, ellipse_size+stroke)
	pygame.draw.ellipse(screen, ellipse_void, ellipse_stroke)
	# ellipse
	ellipse_rect = pygame.Rect(x, y, ellipse_size, ellipse_size)
